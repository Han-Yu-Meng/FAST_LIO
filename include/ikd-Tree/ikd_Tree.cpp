...
{
    static thread_local MANUAL_HEAP q(128); 
    if (q.capacity() < 2 * k_nearest)
    {
        q.resize(2 * k_nearest);
    }
    ...
    {
        Nearest_Points[k_found - i - 1] = q.top().point;
        Point_Distance[k_found - i - 1] = q.top().dist;
        q.pop();
    }
    return;
}
