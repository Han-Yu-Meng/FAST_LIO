                         ...
                         int i_sub_cloud, int num_sub_cloud, double &start_time, double &end_time)
{
  ...
  default:
    fins_node->logger->error("Unsupported lidar type");
    break;
  }
  *pcl_out = pl_surf;
}

void Preprocess::avia_handler(const livox_driver2::msg::CustomMsg::ConstSharedPtr &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  ...
  if (feature_enabled)
  {
    ...
    fins_node->logger->info("Feature extraction time: {} s", time / count);
  }
  else
  {
    for(uint i=1; i<plsize; i++)
...
void Preprocess::give_feature(pcl::PointCloud<PointType> &pl, vector<orgtype> &types)
{
  ...
  if(plsize == 0)
  {
    fins_node->logger->error("Point cloud size is zero in give_feature");
    return;
  }
  uint head = 0;
