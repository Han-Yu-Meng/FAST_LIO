...
void Preprocess::process(const sensor_msgs::msg::PointCloud2::UniquePtr &msg, PointCloudXYZI::Ptr& pcl_out)
{
  ...
  switch (lidar_type)
  {
    ...
    case RSM1_BREAK:
    {
      ...
      robosenseM1_handler(msg, 0, 1, start_time, end_time);
      break;
    }
                                     ...
                                     int i_sub_cloud, int num_sub_cloud, double &start_time, double &end_time)
{
    ...
    // Check if point cloud has ring and timestamp fields
    bool has_ring = false;
    ...
    // Fallback: no ring/timestamp fields, use standard PointXYZI
            ...
            if (feature_enabled) {
                ...
                for (int i = 0; i < plsize; i++) {
                    ...
                    if (range < 150 && range > blind) {
                        ...
                        added_pt.curvature = 0.0;
                        int estimated_ring = (i / points_per_scan) % N_SCANS;
    ...
    // Normal path: has ring and timestamp fields
